...
void createString(String& s0, const char *p)
{
    // Hossz meghatározása
    unsigned int l = 0;
    for (const char* mov = p; *mov != 0; mov++)
    {
        l++;
    }
    s0.len = l;
...
{
    if (idx >= s0.len) throw "index hosszon kivul";
    return s0.pData[idx];
}
 ...
 */
 String addString(const String& s0, const String& s1) {
    String kimenet;
    kimenet.len = s0.len + s1.len;

    kimenet.pData = new char[kimenet.len + 1];

    for (unsigned int i = 0; i < s0.len; i++)
    {
        kimenet.pData[i] = s0.pData[i];
    }

    for (unsigned int i = 0; i < s1.len; i++)
    {
        kimenet.pData[s0.len + i] = s1.pData[i];
    }

    kimenet.pData[kimenet.len] = 0;
...
String addString(const String& s0, char ch) {
    String kimenet;
    kimenet.len = s0.StringLen() + 1;
    kimenet.pData = new char[kimenet.len + 1];
    for (unsigned int i = 0; i < s0.len; i++)
    {
        kimenet.pData[i] = s0.pData[i];
    }
    kimenet.pData[kimenet.len - 1] = ch;
    kimenet.pData[kimenet.len] = 0;
    return kimenet;
}

/**
 * String kiiratásához túlterhelt operátor.
 * @param ostr - referencia az ostream-re
 * @param s0 - referencia a kiiratandó String-re
 * @return ostream& 
 */
std::ostream& operator<<(std::ostream& ostr, const String& s0)
{
    for (unsigned int i = 0; i < s0.len; i++)
    {
        std::cout << s0.pData[i];
