...
int main() {
  ...
  Resistor r;                             // Konstruktorhívás

  TEST(Resistor, HazibanAdottR) {
    double d = r.getR();
    EXPECT_EQ(127., d) << "Jporta feladatban megadott default ertekkel hasonlitson!" << std::endl;
  } END

  Resistor r1(1000), r2(3000);            // Konstruktorhívás
  ...
  TEST(Resistor, Osszeg) {                 // Összegképzéshez konstruktorhívás
  ...
  TEST(Resistor, SetDef) {
    Resistor::setDef(20);
    Resistor rt[5];                         // Tömbelemként 5x konstruktorhívás
